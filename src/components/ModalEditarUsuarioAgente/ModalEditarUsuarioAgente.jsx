...
export const ModalEditarUsuarioAgente = ({ open, onClose, cliente, onGuardar }) => {
  ...
  useEffect(() => {
    if (cliente && open) {
      setFormData({ 
        ...cliente,
        tipo: 2, // Asegurar que siempre sea tipo 2
        rol: 'cliente' // Asegurar que siempre sea rol cliente
      });
    }
  }, [cliente, open]);
  ...
  const handleGuardar = async () => {
    ...
    if (!formData.id_usuario) {
      alert("Error: No se puede editar el cliente sin un ID válido");
      return;
    }

    // Asegurar que los valores fijos se mantengan
    const dataToSend = {
      ...formData,
      tipo: 2,
      rol: 'cliente'
    ...
    try {
      const response = await fetch(`http://35.172.129.60:3030/usuario/editar/${formData.id_usuario}`, {
        method: "PUT",
      ...
      if (response.ok) {
        ...
        alert(data.error || "Error al editar cliente");
      }
    } catch (error) {
...
ModalEditarUsuarioAgente.propTypes = {};